...
5. 除非是全屏页面、弹框等非典型页面类型，否则必须先判断当前任务是 PC 还是移动端页面，并使用对应的基础结构构建页面框架。对于大部分 PC 页面，均需要遵循顶部导航、左侧菜单和中间内容区域的标准结构，且不要对组件和样式进行修改；若任务目标是移动端页面，则必须切换到移动端页面规范。PC 页面基础框架请阅读 [reference/pages/pc.md](reference/pages/pc.md)，移动端页面规范请阅读 [reference/pages/mobile.md](reference/pages/mobile.md)
6. **在能提升清晰度的地方考虑使用动效。** 对于交互状态变化（展开/收起、切换、焦点滑动、错误反馈），优先使用 life-ds 组件内置的动效。仅对非组件元素使用自定义动效，并遵循 [reference/foundations/motion.md](reference/foundations/motion.md) 的预设以保持一致性。
7. **遵循现有项目模式。** 如果仓库中已经使用了 life-ds 组件或本地封装组件，请在其基础上进行扩展，而不是创建平行的结构。
8. 每个项目的页面，需要增加字体平滑的代码：`-webkit-font-smoothing: antialiased`
9. **生成任何页面前，必须先阅读设计原则文档。** 在生成、修改或优化任何页面之前，必须先阅读 [reference/foundations/design.md](reference/foundations/design.md)，并以其中的设计原则作为页面实现的底层依据，确保页面框架、组件使用、信息层级、视觉样式与交互表达均符合设计原则后才可继续实现。
...
> 本项目在 `reference/` 下的 `foundations/`、`pages/` 与 `components/` 目录下分别维护设计规范和组件文档。在实现任何 UI（如按钮、筛选器、表格、布局）之前，你 **必须** 使用文件读取工具查阅对应的 `.md` 参考文件，并优先使用 `@life-ds/components-web` 的 React 组件 API。禁止凭空猜测组件实现，禁止手写 `.lds-` DOM/class 结构替代现有 React 组件。
...
- 设计原则：[reference/foundations/design.md](reference/foundations/design.md)
- token样式：[reference/foundations/tokens.md](reference/foundations/tokens.md)
- 图标：[reference/components/icon.md](reference/components/icon.md)
- 颜色：[reference/foundations/colors.md](reference/foundations/colors.md)
- 组件总览：[reference/components/index.md](reference/components/index.md)
- 动效：[reference/foundations/motion.md](reference/foundations/motion.md)
...
当准备生成页面时，你需要先判断当前任务是 PC 页面还是移动端页面，再选择对应的页面规范。不要默认所有页面都套用同一套布局。

- PC 页面规范：适用于带顶部导航、左侧菜单和中间内容区域的桌面端后台页面，请阅读 [reference/pages/pc.md](reference/pages/pc.md)
- 移动端页面规范：适用于移动端页面、窄屏交互和移动端内容编排，请阅读 [reference/pages/mobile.md](reference/pages/mobile.md)
...
- 按钮 ：[reference/components/button.md](reference/components/button.md)
- 提示条 ：[reference/components/alert.md](reference/components/alert.md)
- 卡片 ：[reference/components/card.md](reference/components/card.md)
- 键值对 ：[reference/components/keyvalue.md](reference/components/keyvalue.md)
- 全局提示 ：[reference/components/message.md](reference/components/message.md)
- 图标 ：[reference/components/icon.md](reference/components/icon.md)
- 选择器 ：[reference/components/select.md](reference/components/select.md)
- 日期选择器 ：[reference/components/datepicker.md](reference/components/datepicker.md)
- 时间选择器 ：[reference/components/timepicker.md](reference/components/timepicker.md)
- 标签页 ：[reference/components/tabs.md](reference/components/tabs.md)
- 步骤条 ：[reference/components/steps.md](reference/components/steps.md)
- 分页器 ：[reference/components/pagination.md](reference/components/pagination.md)
- 表格 ：[reference/components/table.md](reference/components/table.md)
- 筛选器 ：[reference/components/filter.md](reference/components/filter.md)
- 标签 ：[reference/components/tag.md](reference/components/tag.md)
- 多选框 ：[reference/components/checkbox.md](reference/components/checkbox.md)
- 单选框 ：[reference/components/radio.md](reference/components/radio.md)
- 开关 ：[reference/components/switch.md](reference/components/switch.md)
- 搜索框 ：[reference/components/search.md](reference/components/search.md)
- 页面标题区 ：[reference/components/pageheader.md](reference/components/pageheader.md)
- 抽屉 ：[reference/components/drawer.md](reference/components/drawer.md)
- 表单 ：[reference/components/form.md](reference/components/form.md)
...
- [reference/foundations/image-analysis.md](reference/foundations/image-analysis.md)
...
- 动效用于阐明状态变化，而不是用于装饰
- 代码实现遵循了仓库现有的规范
- 所有页面的所有元素均已依据 [reference/foundations/design.md](reference/foundations/design.md) 的设计原则重新 review；凡是不符合设计原则的布局、组件、样式、层级或交互，均已修改至符合要求后才交付
