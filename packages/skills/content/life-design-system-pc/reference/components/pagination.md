...
- **显隐稳定**：分页器一旦被业务选择使用，就不应因当前只剩 1 页而自动消失，以避免切换每页条数后位置跳变；是否使用分页器应由业务在页面层决定，而不是组件在运行时自行隐藏。
...
export default function Demo() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {/* ✅ 单页也保留分页器，避免切换每页条数后位置跳变（默认行为） */}
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <Pagination total={5} defaultPageSize={10} />
      </div>
...
- 显隐逻辑 - 组件不处理“单页自动隐藏”；分页器一旦渲染出来，就应保持位置稳定，不因每页条数切换而突然消失。
