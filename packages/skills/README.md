...
**命令效果**：
该命令会自动将 `content/` 中的每个技能目录分别安装到当前项目工作区的 `.trae/skills/` 下，并同步共享规范目录，例如：

- `.trae/skills/life-design-system-pc/`
- `.trae/skills/life-design-system-mobile/`
- `.trae/skills/_shared/`
- `.trae/skills/prd-design-summarizer/`

### 2. 在 Trae 中使用

执行完上述命令后，重启或刷新 Trae。你将在右侧面板看到 `life-design-system-pc` 与 `life-design-system-mobile` 两个本地技能：桌面端后台页面选择前者，移动端 H5 / React 页面选择后者。
...
安装完成后，Claude Code 会在当前项目中自动发现 `life-design-system-pc` 与 `life-design-system-mobile`。可以让 Claude 按需求自动选择，或手动通过对应名称调用。
...
这样安装后，`life-design-system-pc` 与 `life-design-system-mobile` 会对当前用户的所有 Claude Code 项目生效。

#### 说明

- Claude Code 的技能目录入口文件是 `SKILL.md`，我们同时复制了 `reference/` 下的 `foundations/`、`pages/`、`components/` 文档，以及根目录的 `examples/`、`scripts/`，便于技能按需读取详细规范。
...
npx @life-ds/skills get life-design-system-pc
npx @life-ds/skills get life-design-system-mobile
```
...
### 1. 修改技能
所有的技能源码均位于根目录的 `skills/` 文件夹下，例如 `skills/life-design-system-pc/`、`skills/life-design-system-mobile/`；二者共享的 Token、颜色与图标规范维护在 `skills/_shared/`。
- `skills/<skill-name>/SKILL.md`: 核心指令定义。
- `skills/<skill-name>/reference/foundations/`: 基础规范文档。
- `skills/<skill-name>/reference/pages/`: 页面与布局文档。
- `skills/<skill-name>/reference/components/`: 组件文档。
- `skills/<skill-name>/examples/`: 案例文档。
- `skills/<skill-name>/scripts/`: 常用脚本说明。
- `skills/<skill-name>/gotchas.md`: 常见陷阱。
