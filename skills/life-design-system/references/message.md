...
- 组件已内置多条消息自动堆叠与退出时的平滑上移能力，业务侧直接使用即可，不需要再自行实现一套堆叠动画。
- 组件已内置轻量的进入 / 退出动效，业务侧直接调用 `message` API 或 `<Message>` 即可，不需要额外再补一套展示动画。
...
- **退出节奏**：组件已内置退出时的平滑上移动画，业务使用时无需自行处理堆叠位移。
- **可关闭场景**：当 `closable` 为 `true` 时，需要显示右侧关闭按钮，并支持键盘可访问性。
- **文本约束**：文案以一行为主，避免将复杂描述、链接或操作区塞进 `Message`。
- **动效一致性**：组件已内置进入 / 退出动画和 `prefers-reduced-motion` 兜底，业务层不需要重复实现。
