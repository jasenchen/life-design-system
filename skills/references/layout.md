...
布局所需的核心 CSS 类名（如 `.app-container`, `.app-navbar`, `.app-sidebar`, `.app-body` 等）**已经内置于 `base.css` 中**，您可以直接使用。

实现结构请参考：references/sample-page.html，其中 `app-body` 里的内容仅为参考内容，其他部分则为固定内容。
...
4. **阴影处理**：`.app-body` 的 `z-index` 高于侧边栏和导航栏，以保证 `shadow-large` 投影效果正常显示。
